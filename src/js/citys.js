...
;(function (factory) {
    ...
    $.fn.citys = function(parameter,getApi) {
        ...
        var defaults = {
            dataUrl:'http://7xj2zq.com1.z0.glb.clouddn.com/city-list.json',     //数据库地址
                ...
                success:function(data){
                    ...
                    var format = {
                        province:function(){
                            $province.empty();
                            if(!options.required){
                                $province.append('<option value=""> - 不限 - </option>');
                            }
                            for(i in province){
                                $province.append('<option value="'+province[i]+'" id="'+i+'">'+province[i]+'</option>');
                            }
                            if(options.province){
                        ...
                        city:function(){
                            ...
                            for(i in city){
                                $city.append('<option value="'+city[i]+'" id="'+i+'">'+city[i]+'</option>');
                            }
                            if(options.city){
                        ...
                        area:function(){
                            ...
                            for(i in area){
                                $area.append('<option value="'+area[i]+'" id="'+i+'">'+area[i]+'</option>');
                            }
                            if(options.area){
                    ...
                    $province.on('change',function(){
                        options.province = $(this).find("option:checked").attr('id');
                        options.city = 0;
                        options.area = 0;
                        updateData();
                        format.city();
                    ...
                    $city.on('change',function(){
                        options.city = $(this).find("option:checked").attr('id');
                        options.area = 0;
                        updateData();
                        format.area();
                        options.onChange(_api.getInfo());
                    ...
                    $area.on('change',function(){
                        options.area = $(this).find("option:checked").attr('id');
                        options.onChange(_api.getInfo());
                    });
                    //初始化
                    updateData();
